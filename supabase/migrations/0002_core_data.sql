...
--
-- Structured as ALL TABLES first, then ALL RLS/policies/triggers/views.
-- members_select's policy references ministry_members, and ministry_members
-- itself has a foreign key into members — a straight top-to-bottom
-- table-then-policy layout can't satisfy both directions at once (a policy
-- expression is validated against the schema at CREATE time, like a check
-- constraint, not deferred to query time), so table creation and policy
-- creation are separated into two passes instead.
...
-- ============================================================
-- PASS 2: RLS, POLICIES, TRIGGERS, VIEWS
-- (all tables above now exist, so cross-table policy references resolve)
-- ============================================================

-- FIX: Phase 1 gave Pastor write access to ministries "for all", but the
-- actual Groups/Ministries matrix row is Pastor: View all, not Full.
-- Correcting here rather than silently leaving it — Super Admin only.
