  ...
  return (
    <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      ...
      <div className="glass card" style={{ padding: 14, display: "flex", flexDirection: "column", gap: 10, maxHeight: 640 }}>
        ...
        {showManual && canManageContacts && (
          <div className="row" style={{ gap: 6 }}>
            <button className="btn btn-ghost" style={SMALL_BTN} onClick={() => setShowImport(true)}><Icon name="upload" size={13} /> Import CSV</button>
            <button className="btn btn-ghost" style={SMALL_BTN} onClick={() => exportContactsCsv(manualCandidates)}><Icon name="download" size={13} /> Export CSV</button>
          </div>
        )}
      ...
      {showImport && (
        <ManualContactsImportModal
          existingPhones={(manualContacts ?? []).map((c) => c.phone)}
...
function exportContactsCsv(contacts) {
  const header = "full_name,phone,email,group_label,notes";
  const rows = contacts.map((c) => [c.full_name, c.phone, c.email ?? "", c.group_label ?? "", c.notes ?? ""].map(csvEscape).join(","));
  const csv = [header, ...rows].join("\n");
  const blob = new Blob([csv], { type: "text/csv" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `manual-contacts-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
...
function ManualContactsImportModal({ existingPhones, onClose }) {
  const [rawText, setRawText] = useState("");
  const [parsed, setParsed] = useState(null);
  const [error, setError] = useState(null);
  const bulkCreate = useBulkCreateManualContacts();
  ...
  const parse = () => {
    ...
    if (lines.length === 0) return setError("Paste or upload CSV content first.");
    const hasHeader = /name/i.test(lines[0]) && /phone/i.test(lines[0]);
    ...
    for (const line of dataLines) {
      ...
      const [fullName, phoneRaw, email, groupLabel, notes] = cols;
      const normalized = normalizeGhanaPhone(phoneRaw);
      if (!fullName || !normalized) {
        invalid.push({ line, reason: !fullName ? "Missing name" : "Invalid phone" });
        continue;
      }
      if (seen.has(normalized)) {
        duplicates.push({ line, phone: normalized });
        continue;
      }
      seen.add(normalized);
      valid.push({ fullName, phone: normalized, email, groupLabel, notes });
    }
    setParsed({ valid, invalid, duplicates });
  };

  const onImport = async () => {
    if (!parsed?.valid.length) return;
    try {
      await bulkCreate.mutateAsync(parsed.valid);
      onClose();
    } catch (err) {
  ...
  return (
    <Modal onClose={onClose}>
      <div className="glass" style={{ width: "100%", maxWidth: 520, padding: 26, maxHeight: "82vh", display: "flex", flexDirection: "column" }} onClick={(e) => e.stopPropagation()}>
        <div className="row between" style={{ marginBottom: 14 }}>
          <h2 style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 500 }}>Import contacts (CSV)</h2>
          <button className="btn btn-icon btn-ghost" onClick={onClose}><Icon name="x" size={16} /></button>
        </div>
        <p className="muted" style={{ fontSize: 12.5, marginBottom: 10 }}>
          Columns: full_name, phone, email, group_label, notes (header row optional).
        </p>
        <input type="file" accept=".csv,text/csv" onChange={onFileChange} style={{ marginBottom: 10 }} />
        <textarea className="textarea" rows={6} placeholder="Or paste CSV rows here…" value={rawText} onChange={(e) => setRawText(e.target.value)} />
        <button className="btn" style={{ marginTop: 10, alignSelf: "flex-start" }} onClick={parse}>Preview</button>

        {parsed && (
          <div style={{ marginTop: 14, overflowY: "auto" }}>
            <div className="row" style={{ gap: 8, marginBottom: 8 }}>
              <span className="badge badge-green">{parsed.valid.length} valid</span>
              <span className="badge badge-gold">{parsed.duplicates.length} duplicate</span>
              <span className="badge badge-red">{parsed.invalid.length} invalid</span>
            </div>
            {parsed.invalid.length > 0 && (
              <div className="muted" style={{ fontSize: 11.5 }}>
                Invalid rows will be skipped ({parsed.invalid.map((r) => r.reason).join(", ")}).
              </div>
            )}
          </div>
        )}
        ...
        <div className="row" style={{ gap: 8, marginTop: 20, justifyContent: "flex-end" }}>
          <button className="btn" onClick={onClose}>Cancel</button>
          <button className="btn btn-primary" onClick={onImport} disabled={!parsed?.valid.length || bulkCreate.isPending}>
            Import {parsed?.valid.length ?? 0} contact{parsed?.valid.length === 1 ? "" : "s"}
          </button>
        </div>
      </div>
    </Modal>
