...
import { useMembers, useAllMemberContacts, useBulkCreateMembers, useUpdateMember } from "../../hooks/useMembers.js";
import {
...
import {
  ...
  useBulkCreateManualContacts, useAllManualContactPhones,
...
function SendTab({ role, profile, access, config, prefillBody, onConsumePrefill }) {
  ...
  // Members import/export gated to match members_insert RLS exactly
  // (0002_core_data.sql) — not the messages-module permission above.
  const canImportMembers = role === "super_admin" || role === "secretary";
  const allowedImportTargets = [
    ...(canManageContacts ? ["manualContacts"] : []),
    ...(canImportMembers ? ["members"] : []),
  ];
  ...
  return (
    <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 14 }}>
      ...
      <div className="glass card" style={{ padding: 14, display: "flex", flexDirection: "column", gap: 10, maxHeight: 640 }}>
        ...
        {allowedImportTargets.length > 0 && (
          <div className="row" style={{ gap: 6, flexWrap: "wrap" }}>
            ...
            <button className="btn btn-ghost" style={SMALL_BTN} onClick={() => setShowExport(true)}><Icon name="download" size={13} /> Export CSV</button>
          </div>
        )}
      ...
      {showImport && (
        <CsvImportModal
          allowedTargets={allowedImportTargets}
      ...
      {showExport && (
        <ExportCsvModal
          allowedTargets={allowedImportTargets}
          filteredMembers={candidates}
          filteredManualContacts={manualCandidates}
          selectedMembers={Array.from(selectedMembers.values())}
          selectedManualContacts={Array.from(selectedManual.values())}
...
function CsvImportModal({ allowedTargets, ministries, onClose }) {
...
function ExportCsvModal({ allowedTargets, filteredMembers, filteredManualContacts, selectedMembers, selectedManualContacts, onClose }) {
  const [targetKey, setTargetKey] = useState(allowedTargets[0]);
